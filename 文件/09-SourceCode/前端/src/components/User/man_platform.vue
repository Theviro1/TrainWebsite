        ...
        <el-col :span="4">
          ...
          <el-dialog :visible.sync="dialogVisible" title="添加经停站信息" width="30%" @close="addDialogClosed('form')">
            <el-form ref="form" :model="addForm" :rules="addPlatformRules" label-width="100px">
              <el-form-item label="到达顺序" prop="id" label-width = "10%">
...
export default {
  ...
  methods: {
    async getplatformsList(trainid) {
      await axios
        .get('http://123.57.3.84/train/schedule', { params: { id: trainid } })
        .then((res) => {
          console.log(1)
          console.log(res)
          const { data } = res.data
    ...
    async searchplatformList(query_a) {
      await axios.get('http://123.57.3.84/schedule/search/id', { params: { trainId: this.$store.state.trainid, id: query_a } }).then((res) => {
        console.log(1)
        console.log(res)
        const { data } = res.data
        console.log(data)
      ...
      if (this.compareTime(this.addForm.reachTime, this.addForm.leaveTime)) {
        this.$refs.form.validate(async (valid) => {
          ...
          if (!valid) {
            ...
            console.log(this.addForm)
            await axios
              .post("http://123.57.3.84/train/schedule/add", this.addForm)
              .then((res) => {
                console.log(res)
      ...
      if (this.compareTime(this.editForm.reachTime, this.editForm.leaveTime)) {
        this.$refs.Edit_Form.validate(async (valid) => {
          ...
          if (!valid) {
            ...
            console.log(this.editForm)
            await axios
              .put('http://123.57.3.84/train/schedule/update', this.editForm)
              .then((res) => {
                console.log(res)
                if (res.data.success == true) {
                  this.$message({
