  ...
  methods: {
    ...
    async ticketfastQuery(){
      if(this.time ==''){
        ...
        await axios.get('http://123.57.3.84/train/search/fastest',
    ...
    async ticketQuery() {
      if (!this.time.length) {
        ...
        await axios.get('http://123.57.3.84/train/search1', { params: { departurePlatformId: this.departureid, arrivalPlatformId: this.arrivalid } }).then((res) => {
          console.log(res)
          const { data } = res.data
          this.trainsList = data
        })
        ...
        var date1Str = date1.toISOString().slice(0, -5); // 转换并去掉最后5个字符 
        var date2Str = date2.toISOString().slice(0, -5); // 转换并去掉最后5个字符
        await axios.get('http://123.57.3.84/train/search',
          { params: { departurePlatformId: this.departureid, arrivalPlatformId: this.arrivalid, departureDate1: date1Str, departureDate2: date2Str } }).then((res) => {
            const { data } = res.data
            this.trainsList = data
          })
